...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($nama_genre == '') {
        ...
        header('Location: genre_tambah.php');
        exit();
    }

    $insert = mysqli_query($conn, "INSERT INTO tb_genre (nama_genre) VALUES ('$nama_genre')");

    if ($insert) {
        $_SESSION['flash'] = 'Genre baru berhasil ditambahkan!';
        $_SESSION['flash_type'] = 'success';
        header('Location: genre_data.php');
        exit();
    } else {
        $_SESSION['flash'] = 'Gagal menambahkan genre baru.';
        $_SESSION['flash_type'] = 'error';
        header('Location: genre_tambah.php');
        exit();
    }
...
<head>
    ...
    <title>Tambah Genre - ArsipMusik</title>
        ...
        <div class="section">
            <div class="container-admin" style="max-width: 600px;">
                <div class="admin-header-tesla">
                    <h1>Tambah Genre</h1>
                    <p>Tambah kategori musik baru untuk koleksi Anda</p>
                </div>
                ...
                <div class="form-card" style="margin: 0; padding: 0; border: none; box-shadow: none; background: transparent;">
                    <form action="genre_tambah.php" method="POST" style="margin: 0; padding: 25px; background: #fff; border: 1px solid var(--border-color); border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
                        ...
                        <div class="form-actions" style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #f1f5f9; display: flex; justify-content: flex-end; gap: 12px;">
                            ...
                            <button type="submit" style="width: auto; padding: 0 30px; height: 44px; border-radius: 8px; background: var(--primary); color: #fff; font-weight: 600; border: none; cursor: pointer; transition: all 0.2s ease;">Simpan Genre</button>
                        </div>
                    </form>
                </div>
            </div>
